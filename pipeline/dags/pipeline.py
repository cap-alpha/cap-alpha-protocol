...
with DAG(
    ...
    set_year >> [
        scrape_spotrac_team_caps,
        scrape_pfr_rosters,
        scrape_spotrac_player_rankings,
    ]
    ...
    (
        generate_curated_contracts >> stage_spotrac_contracts
    )  # Use generated contracts as primary source

    # Layer 3 (Normalization) - depends on all staging
    [
        stage_spotrac_team_caps,
        stage_pfr_rosters,
        stage_spotrac_rankings,
        stage_spotrac_contracts,
    ] >> normalize_data
    ...
    (
        [data_quality_checks, validate_dead_money]
        >> run_feature_factory
        >> train_risk_model
    )
