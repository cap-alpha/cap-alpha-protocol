...
def calculate_historical_savings():
    ...
    try:
        ...
        df["dead_cap_avoided_millions"] = df["dead_cap_avoided_millions"].apply(
            lambda x: f"${x:,.2f}M"
        )
        if "false_positive_opportunity_cost_millions" in df.columns:
            df["false_positive_opportunity_cost_millions"] = df[
                "false_positive_opportunity_cost_millions"
            ].apply(lambda x: f"${x:,.2f}M" if pd.notnull(x) else "$0.00M")
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                "*If a General Manager had access to the Cap Alpha Protocol during the 2019-2024 testing windows, how much toxic Cap Space would they have saved by vetoing predicted 'Bust' contracts?*\n\n"
            )
            f.write("### Methodology\n")
            f.write(
                "We joined our `is_bust_binary` 97% Accuracy classification predictions (generated exclusively on lagged data) against the absolute financial ledgers for that year. We define 'Dead Cap Avoided' as True Positives: The player was classified as a Bust, and subsequently failed to meet a 0.70x baseline ROI hurdle on the field.\n\n"
            )
            f.write("```text\n")
