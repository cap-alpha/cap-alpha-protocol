...
def check_leakage():
    ...
    if top_1_imp > 0.50:
        logger.error(
            f"🚨 EXTREME LEAKAGE RISK: '{df_imp.iloc[0]['feature']}' accounts for {top_1_imp * 100:.1f}% of model splits!"
        )
        sys.exit(1)
    elif top_2_imp > 0.80:
        logger.warning(
            f"⚠️ POTENTIAL LEAKAGE: Top 2 features account for {top_2_imp * 100:.1f}% of splits."
        )
    else:
        logger.info(
            "✅ Feature importance looks distributed normally. No single feature dominates >50%."
        )
    ...
    for c in corrs:
        if abs(c["correlation_with_target"]) > 0.85:
            logger.error(
                f"🚨 EXTREME LEAKAGE RISK: '{c['feature']}' has a {c['correlation_with_target']:.3f} correlation with target!"
            )
            sys.exit(1)
