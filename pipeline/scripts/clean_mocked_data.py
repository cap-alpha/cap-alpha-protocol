...
def main():
    ...
    try:
        df = db.fetch_df(
            "SELECT * FROM media_lag_metrics WHERE rationale LIKE '%Mocked%'"
        )
    ...
    try:
        ...
        df = db.fetch_df(
            "SELECT * FROM information_schema.columns WHERE table_name='prediction_results' AND column_name='rationale'"
        )
        if len(df) > 0:
            mocked_preds = db.fetch_df(
                "SELECT * FROM prediction_results WHERE rationale LIKE '%Mocked%'"
            )
            if len(mocked_preds) > 0:
                print(f"Found {len(mocked_preds)} in prediction_results")
                db.execute(
                    "DELETE FROM prediction_results WHERE rationale LIKE '%Mocked%'"
                )
    except Exception as e:
        pass
