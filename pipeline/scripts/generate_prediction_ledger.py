...
def generate_ledger(year: int):
    ...
    with DBManager() as db:
        ...
        try:
            ...
            if df.empty:
                logger.warning(
                    f"No records found in fact_player_efficiency for {year}. Cannot generate ledger."
                )
                return
        ...
        for _, row in df.iterrows():
            payload = {
                ...
                "potential_dead_cap_millions": row.get(
                    "potential_dead_cap_millions", 0
                ),
            ...
            signature_base = (
                f"{row['player_name']}|{row['year']}|{current_time}|{payload_str}"
            )
        ...
        logger.info(
            f"Hashing complete. Appending {len(df_ledger)} cryptographic records to BigQuery Ledger..."
        )
