...
def generate_frontier_plot():
    ...
    df["combined_roi_score_capped"] = df["combined_roi_score"].clip(
        upper=df["combined_roi_score"].quantile(0.95)
    )
    ...
    for h in highlights:
        ...
        if not player_data.empty:
            ...
            plt.annotate(
                ...
                fontsize=12,
                bbox=dict(
                    boxstyle="round,pad=0.3", fc=h["color"], alpha=0.3, ec="white", lw=1
                ),
                arrowprops=dict(
                    arrowstyle="->", connectionstyle="arc3,rad=0.1", color="white"
                ),
            )

    # 2. Add Zone Labels
    plt.text(
        df["cap_hit_millions"].max() * 0.1,
        df["combined_roi_score_capped"].max() * 0.9,
    ...
    plt.text(
        df["cap_hit_millions"].max() * 0.7,
        df["combined_roi_score_capped"].min() * 1.1,
        "THE TOXIC DEBT ZONE\n(Ineligible ROI, High Risk)",
        color="#ff4d4d",
        fontsize=16,
        fontweight="bold",
        alpha=0.8,
    ...
    plt.ylabel(
        "Combined ROI Score (Football + Commercial)",
        fontsize=14,
        fontweight="bold",
        color="gray",
    )
    plt.title(
        "THE 2025 DISCIPLINE FRONTIER: Realized ROI vs. Cap Exposure",
        fontsize=22,
        fontweight="bold",
        pad=30,
    )
    ...
    cbar.set_label(
        "EDCE Risk Score", rotation=270, labelpad=20, fontsize=12, fontweight="bold"
    )
