...
class NewsRSSHoover:
    ...
    def gather_team_news(self, team_name: str, players: list) -> bool:
        ...
        logger.info(
            f"Hoovering Franchise-Level Google News intelligence for {team_name}..."
        )
        try:
            # URL encode the search query for the franchise
            query = urllib.parse.quote(
                f'"{team_name}" NFL (injury OR rumor OR contract OR trade) -fantasy'
            )
            ...
            logger.info(
                f"  Found {len(feed.entries)} macro news articles for {team_name}."
            )
            ...
            for player in players:
                ...
                last_name = (
                    player.split()[-1].lower()
                    if len(player.split()) > 1
                    else player.lower()
                )

                if player.lower() in compiled_content or (
                    len(last_name) > 3 and last_name in compiled_content
                ):
                    hits += 1
            ...
            logger.info(
                f"✅ Successfully cross-referenced {hits} players out of {len(players)} active roster names for {team_name}."
            )
            return True
...
def get_active_rosters():
    ...
    try:
        ...
        logger.info(
            f"Successfully loaded {len(rosters)} active franchises for batch processing."
        )
        return rosters
    except Exception as e:
