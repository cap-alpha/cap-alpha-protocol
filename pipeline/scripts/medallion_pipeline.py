...
try:
    ...
    FINANCIAL_INGESTION_AVAILABLE = True
except ImportError as e:
    logging.getLogger(__name__).warning(
        "financial_ingestion module unavailable: %s. "
        "Pipeline will skip financial data loading.",
        e,
    )
    load_team_financials = None  # type: ignore
    load_player_merch = None  # type: ignore
    FINANCIAL_INGESTION_AVAILABLE = False
...
class SilverLayer:
    ...
    def ingest_others(self):
        ...
        if fin_path.exists() and FINANCIAL_INGESTION_AVAILABLE:
            load_team_financials(self.db.con, fin_path)
        elif fin_path.exists():
            logger.warning(
                "Financial data available but financial_ingestion module is unavailable — skipping."
            )

        merch_path = BRONZE_DIR / "other" / "merch" / "nflpa_player_sales_2024.csv"
        if merch_path.exists() and FINANCIAL_INGESTION_AVAILABLE:
            load_player_merch(self.db.con, merch_path)
        elif merch_path.exists():
            logger.warning(
                "Merch data available but financial_ingestion module is unavailable — skipping."
            )
