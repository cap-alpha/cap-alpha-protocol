...
def run_analysis():
    ...
    print("\n--- Carlton Davis Stats ---")
    davis = full_df[
        full_df["player_name_short"]
        .astype(str)
        .str.contains("C.Davis", case=False, na=False)
    ]
    if not davis.empty:
        print(
            davis[
                ["player_name_short", "team_city", "penalty_yards", "year"]
            ].to_string()
        )
    else:
    ...
    if "team_city" in full_df.columns and "penalty_yards" in full_df.columns:
        team_stats = (
            full_df.groupby(["team_city", "year"])["penalty_yards"].sum().reset_index()
        )
        ...
        target_teams = [
            ...
            "Detroit",
            "Denver",
        ]  # Including DEN/DET for context
        print(
            team_stats[team_stats["team_city"].isin(target_teams)].sort_values(
                ["year", "penalty_yards"]
            )
        )
        ...
        for year in team_stats["year"].unique():
            print(f"\nYear: {year}")
            year_data = team_stats[team_stats["year"] == year].sort_values(
                "penalty_yards"
            )
