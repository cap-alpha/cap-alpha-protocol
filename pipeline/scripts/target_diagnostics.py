...
def run_diagnostics():
    with DBManager() as db:
        logger.info(
            "Fetching Gold Matrix (`fact_player_efficiency`) for Diagnostic Analysis..."
        )
        try:
        ...
        print(
            f"Kurt:   {ratio_clean.kurtosis():.2f} (Targeting < 3.0 to avoid fat tails)"
        )
        ...
        pos_var = (
            df.groupby("position")["efficiency_ratio"]
            .std()
