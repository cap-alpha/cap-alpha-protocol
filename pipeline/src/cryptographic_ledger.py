...
def hash_predictions_to_ledger():
    ...
    try:
        latest = db.fetch_df("SELECT MAX(year) as y, 0 as w FROM prediction_results").to_dict('records')
        ...
        existing = db.fetch_df(f"SELECT block_id FROM audit_ledger_blocks WHERE year = {year} AND week = {week}").to_dict('records')
        ...
        for p in preds:
            payload = {
                ...
                "year": year,
                "week": week,
                "risk_score": float(p['predicted_risk_score']) if pd.notna(p['predicted_risk_score']) else 0.0,
                "fmv": 0.0,
                "edce": 0.0,
                "timestamp": timestamp
            }
