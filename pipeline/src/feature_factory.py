...
class FeatureFactory:
    ...
    def _validate_point_in_time(self, df):
        ...
        for player in sample_players:
            ...
            for i in range(1, len(player_df)):
                ...
                if (
                    pd.isna(current_year)
                    or pd.isna(current_week)
                    or pd.isna(lag_year)
                    or pd.isna(lag_week)
                ):
                    continue
                ...
                if lag_year > current_year or (
                    lag_year == current_year and lag_week >= current_week
                ):
                    violations += 1
                    logger.warning(
                        f"⚠️ Violation: {player} has lag data from {lag_year}-W{lag_week} for week {current_year}-W{current_week}"
                    )

        if violations == 0:
            logger.info(
                "✅ Point-in-Time validation PASSED: No future data leakage detected."
            )
        else:
            logger.warning(
                f"⚠️ Point-in-Time validation: {violations} potential violations found (may be gaps)."
            )
    ...
    def generate_hyperscale_matrix(self):
        ...
        df["injury_gap_weeks"] = (
            (df["abs_week"] - df.groupby("player_name")["abs_week"].shift(1) - 1)
            .clip(lower=0)
            .fillna(0)
        )
        ...
        if "total_tds_lag_1" in df.columns:
            df["td_per_dollar"] = df["total_tds_lag_1"] / df[
                "cap_hit_millions"
            ].replace(0, np.nan)
        ...
        if all(
            c in df.columns
            for c in ["total_tds_lag_1", "total_tds_lag_2", "total_tds_lag_3"]
        ):
            df["td_volatility"] = df[
                ["total_tds_lag_1", "total_tds_lag_2", "total_tds_lag_3"]
            ].std(axis=1)
        ...
        for category, count in narrative_categories.items():
            for i in range(count):
                sensor_cols[f"sensor_{category}_{i}"] = (
                    np.random.normal(0, 1, size=len(df))
                    * df["sentiment_volume"]
                    / 100.0
                )

        # Attempt to pull true historical NLP embeddings from BigQuery (generated by Gemini)
        try:
            df_nlp = self.db.fetch_df("SELECT * FROM nlp_sentiment_features_true")
            if not df_nlp.empty:
                logger.info(
                    f"🟢 Found {len(df_nlp)} rows of true 768-D NLP Sentiment Vectors. Joining to feature matrix..."
                )
                ...
                logger.info(
                    "✅ True NLP Sentiment Vectors successfully injected into XGBoost matrix. Fake generic noise dropped."
                )
            else:
                logger.warning(
                    "🟡 silver_layer.nlp_sentiment_features_true is empty. Falling back to generic vector noise."
                )
                if sensor_cols:
                    df = pd.concat(
                        [df, pd.DataFrame(sensor_cols, index=df.index)], axis=1
                    )
        except Exception as e:
            logger.warning(
                f"🟡 True NLP embeddings not found in DB ({e}). Injecting baseline vector noise."
            )
            if sensor_cols:
...
if __name__ == "__main__":
    ...
    factory.db.execute(
        "CREATE OR REPLACE TABLE staging_feature_matrix AS SELECT * FROM matrix_df",
        {"matrix_df": matrix},
    )
