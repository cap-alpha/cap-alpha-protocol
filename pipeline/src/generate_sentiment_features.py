...
def main():
    ...
    try:
        ...
        logger.warning(f"Could not load targets from fact_player_efficiency, falling back to prototype list. Error: {e}")
        targets = ["Russell Wilson", "Aaron Rodgers", "Deshaun Watson", "Ezekiel Elliott", "Jamal Adams"]
    ...
    for player in tqdm(targets, desc="Processing Sentiment Batch"):
        ...
        media_text = media_df['raw_text'].iloc[0] if not media_df.empty else "No recent news data available."
        ...
        if not injury_df.empty:
            ...
            injury_text = "Historical Injury Log: " + " | ".join(
                [f"Status: {log.get('report_status', 'Unknown')} due to {log.get('report_primary_injury', 'Unknown')}" for log in injury_logs]
            )
    ...
    if vector_results:
        ...
        db.execute("CREATE OR REPLACE TABLE silver_layer.nlp_sentiment_features_true AS SELECT * FROM df_embeddings", {"df_embeddings": df_embeddings})
        
        # Persist narratives for UI
        db.execute("CREATE OR REPLACE TABLE silver_layer.nlp_intelligence_narratives AS SELECT * FROM df_narratives", {"df_narratives": df_narratives})
        
        logger.info(f"✅ Successfully hydrated Silver Layer for {len(vector_results)} elite targets.")
    else:
