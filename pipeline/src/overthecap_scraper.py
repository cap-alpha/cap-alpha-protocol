...
class OverTheCapScraper:
    ...
    def scrape_team_contracts(self, year: int) -> pd.DataFrame:
        ...
        logger.info(
            f"Scraping Over The Cap contracts for {year} ({len(team_codes)} teams)"
        )
        ...
        for tbl in soup.find_all("table"):
            ...
            if "Player" in headers and "CapNumber" in headers:
                ...
                for row in tbody.find_all("tr"):
                    ...
                    try:
                        contract = self._parse_contract_row(
                            cells, headers, team_code, year
                        )
                        if contract:
    ...
    def _parse_contract_row(
        self, cells: List, headers: List[str], team_code: str, year: int
    ) -> Optional[Dict]:
        ...
        idx_guar = (
            headers.index("Guaranteed Salary") if "Guaranteed Salary" in headers else -1
        )
        idx_prorated = (
            headers.index("Prorated Bonus") if "Prorated Bonus" in headers else -1
        )
        ...
        cap_hit = (
            _parse_money(cells[idx_cap].text)
            if idx_cap != -1 and idx_cap < len(cells)
            else 0.0
        )
        guar_salary = (
            _parse_money(cells[idx_guar].text)
            if idx_guar != -1 and idx_guar < len(cells)
            else 0.0
        )
        signing_bonus = (
            _parse_money(cells[idx_prorated].text)
            if idx_prorated != -1 and idx_prorated < len(cells)
            else 0.0
        )
