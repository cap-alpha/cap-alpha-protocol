...
def run_silver_transformation():
    ...
    """
    logger.info("Starting Silver Transformation: SportsData.io -> silver_player_metadata")
    ...
    with DBManager() as db:
        ...
        df_silver['birth_date'] = pd.to_datetime(df_bronze['BirthDate'], errors='coerce').dt.strftime('%Y-%m-%d')
        
        df_silver['college'] = df_bronze['College']
        df_silver['experience_years'] = pd.to_numeric(df_bronze['Experience'], errors='coerce').fillna(0).astype(int)
        ...
        if 'DraftRound' in df_bronze.columns:
            df_silver['draft_round'] = pd.to_numeric(df_bronze['DraftRound'], errors='coerce').fillna(0).astype(int)
            df_silver['draft_pick'] = pd.to_numeric(df_bronze['DraftPick'], errors='coerce').fillna(0).astype(int)
        else:
        ...
        job = db.client.load_table_from_dataframe(df_silver, table_ref, job_config=job_config)
        job.result()
