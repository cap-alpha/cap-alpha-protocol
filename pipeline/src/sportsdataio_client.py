...
class SportsDataIOCoreClient:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            raise EnvironmentError("SPORTS_DATA_IO_API_KEY is not set in the environment.")
    ...
    def fetch_active_players(self) -> List[Dict[str, Any]]:
        ...
        if response.status_code != 200:
            logger.error(f"Failed to fetch active players. HTTP {response.status_code}: {response.text}")
...
def ingest_bronze_players():
    ...
    logger.info(f"Loaded {len(df_players)} players into memory. Writing to BigQuery `bronze_sportsdataio_players`...")
    ...
    with DBManager() as db:
        ...
        job = db.client.load_table_from_dataframe(df_cleaned, table_ref, job_config=job_config)
        job.result()
        logger.info(f"Successfully truncated and rebuilt table {table_ref} with {len(df_cleaned)} records.")
