...
[[ -f "$PERSONAS" ]] || err ".env.personas not found at $PERSONAS. Create it with LARS_TOKEN and SLACK_WEBHOOK_URL."
grep -q "^LARS_TOKEN=" "$PERSONAS" || err ".env.personas missing LARS_TOKEN"
grep -q "^SLACK_WEBHOOK_URL=" "$PERSONAS" || err ".env.personas missing SLACK_WEBHOOK_URL"
