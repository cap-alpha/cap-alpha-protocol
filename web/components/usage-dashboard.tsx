...
function getTierBadge(tier: string, tierName: string) {
    const styles: Record<string, string> = {
        free: "bg-editorial-border text-ink-2 ring-editorial-border",
        pro: "bg-accent-editorial/10 text-accent-editorial ring-accent-editorial/40",
        agent: "bg-gold/10 text-gold ring-gold/40",
        api_starter: "bg-correct/10 text-correct ring-correct/40",
        api_growth: "bg-pending/10 text-pending ring-pending/40",
        enterprise: "bg-incorrect/10 text-incorrect ring-incorrect/40",
    };
...
function RateLimitBar({
    ...
    const barColor =
        pct > 80 ? "bg-incorrect" : pct > 50 ? "bg-pending" : "bg-correct";

    return (
        <div>
            <div className="flex justify-between mb-1.5">
                <span className="text-xs font-mono text-ink-2">{label}</span>
                <span className="text-xs font-mono text-ink-2 tabular-nums">
                    {formatNumber(current)} / {formatNumber(limit)}
                </span>
            </div>
            <div className="w-full bg-editorial-border rounded-full h-1.5 overflow-hidden">
                <div
                    className={cn(
...
export function UsageDashboard() {
    ...
    if (!data) {
        return (
            <div className="rounded-xl border border-editorial-border bg-editorial-card py-16 text-center text-ink-3 text-sm">
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="rounded-xl border border-editorial-border bg-editorial-card p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div>
                        ...
                        <p className="text-2xl font-black font-mono text-ink tabular-nums">
                    ...
                    <div>
                        ...
                        <p className="text-2xl font-black font-mono text-correct tabular-nums">
                            {successRate}%
                        </p>
                        <p className="text-xs text-ink-3 font-mono mt-0.5">
                            2xx responses
            ...
            {shouldShowUpgradeBanner && (
                <div className="rounded-xl border border-pending/30 bg-pending/10 p-5 flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-pending/80 mt-0.5">
                    ...
                    <a
                        href="/pricing"
                        className="shrink-0 inline-flex items-center gap-1.5 rounded-lg bg-accent-editorial hover:bg-accent-editorial-light transition-colors px-4 py-2 text-xs font-bold text-white uppercase tracking-wide"
                    >
            ...
            {!shouldShowUpgradeBanner && data.currentTier === "free" && (
                <div className="rounded-xl border border-editorial-border bg-editorial-card p-5 flex items-center justify-between gap-4">
                    <div>
            ...
            {data.emptyState && (
                <div className="rounded-xl border border-editorial-border bg-editorial-card p-6">
                    <div className="flex items-center gap-2 mb-3">
                        <Zap className="w-4 h-4 text-accent-editorial" />
                    ...
                    {/* Deliberate dark exception: terminal/code block reads best on dark */}
                    <pre className="bg-zinc-950 border border-zinc-800 rounded-lg p-4 text-xs font-mono text-emerald-300 overflow-x-auto">
            ...
            {!data.emptyState && (
                <div className="rounded-xl border border-editorial-border bg-editorial-card p-6">
                    <div className="flex items-center gap-2 mb-1">
                        <TrendingUp className="w-4 h-4 text-accent-editorial" />
                        <span className="text-xs font-mono uppercase tracking-widest text-accent-editorial">
                            Request History
                        </span>
                    </div>
                    <h3 className="text-lg font-black tracking-tight text-ink mb-1">
                        Last 30 Days
                    </h3>
                    <p className="text-xs text-ink-3 font-mono mb-5">
                    ...
                    {totalRequests > 0 ? (
                        ...
                        <div className="h-40 flex items-center justify-center text-ink-3 text-sm font-mono">
            ...
            <div className="rounded-xl border border-editorial-border bg-editorial-card p-6">
                ...
                <h3 className="text-lg font-black tracking-tight text-ink mb-5">
                    Current Usage
                </h3>
            ...
            {!data.emptyState && data.topEndpoints.length > 0 && (
                <div className="rounded-xl border border-editorial-border bg-editorial-card p-6">
                    ...
                    {/* Column headers */}
                    <div className="grid grid-cols-[1fr_80px_60px] gap-3 px-2 pb-2 text-[10px] font-mono uppercase tracking-widest text-ink-3">
                        <span>Endpoint</span>
                    ...
                    <div className="space-y-1">
                        {data.topEndpoints.map((ep, idx) => (
                            <div
                                key={idx}
                                className="grid grid-cols-[1fr_80px_60px] gap-3 items-center rounded-lg px-2 py-2.5 bg-editorial-bg border border-editorial-border"
                            >
                                <span className="text-xs font-mono text-ink-2 truncate">
                                    {ep.endpoint_path}
                                </span>
                                <span className="text-xs font-mono text-ink text-right tabular-nums">
                                    {formatNumber(ep.count)}
                                </span>
                                <span className="text-xs font-mono text-ink-3 text-right tabular-nums">
                                    {ep.pct}%
                                </span>
                            </div>
                        ))}
